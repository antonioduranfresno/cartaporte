        ...
        <div id="page-wrapper">
		   ...
		   <div id="divCartaPorte" class="tab-pane fade in active">
	           ...
	           <sf:form method="post" modelAttribute="cartaPorte" >
					...
					<div class="row">
			      		<div class="col-sm-4">
			        		<label>Modelo Mercancía</label>
			        		<sf:errors path="modelo.id" class="label label-danger"></sf:errors>
			        		<sf:select class="form-control" id="selModelo" path="modelo.id" value="${cartaPorte.modelo.id}" onchange="cargaEtiquetas();">
								<c:forEach items="${listaModelos}" var="c" varStatus="index">
				        			<option value="${c.id}" ${c.id == cartaPorte.modelo.id ? 'selected' : '' }>${c.toStringDescripcionModelo()}</option>
				        		</c:forEach>
							</sf:select>
			        	</div>						
			...
			<div id="divMercancia" class="tab-pane fade"> 
			
				<jsp:include page="mercancia.jsp"/>
				
			</div>
			
			<div id="divMediosDurables" class="tab-pane fade"> 
				
				<jsp:include page="medioDurable.jsp"/>
